...
class CVEmbedder:
    ...
    def embed_cv(self, cv_json_path, force_reembed=False):
        ...
        try:
            ...
            logger.error(f"Failed to generate CV ID: {e}")
            return False

        # Check if CV already exists (unless force_reembed is True)
        if not force_reembed and self.check_cv_exists(cv_id):
        ...
        if not documents:
            logger.error(f"No documents prepared for cv_id {cv_id}")
            return False
        ...
        success = self.store_documents(valid_docs, valid_embeddings)
        
        if success:
            logger.info(f"✓ Successfully embedded CV {cv_id} with {len(valid_docs)} document chunks")
        else:
            logger.error(f"✗ Failed to store embeddings for CV {cv_id}")
